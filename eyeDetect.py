#-------------------------------------------------------------------------------
# Name:        eyeDetect
# Purpose:     eye detecting
#
# Author:      lukea_000 and his partners
# First Modifier:    galal1
# Firstly Modified parts: Just some parts to fit my gui.py and init needed variable (parts signed)
#
# Created:     02/11/2013
...
gazeCoords = [0, 0]     # modified: init gazeCoords for replay coords on my gui(gui.py) (galal1)
isDoTrainingRunning = False # modified: to have mode for relaunch doTraining for my gui.py (galal1)

def getGazeCoords():    # modified: to have getter/setter gazeCoords and isDoTrainingRunning (galal1)
    return [gazeCoords[0], gazeCoords[1]]

def setGazeCoords(x, y):
    global gazeCoords
    gazeCoords = [x, y]

def getIsDoTrainingRunning():
    return isDoTrainingRunning

def setIsDoTrainingRunning(isItRunning):
    global isDoTrainingRunning
    isDoTrainingRunning = isItRunning
...
haarFaceCascade = cv2.CascadeClassifier("./datas/haarcascades/haarcascade_frontalface_alt.xml")
haarEyeCascade = cv2.CascadeClassifier("./datas/haarcascades/haarcascade_eye.xml")
...
def main():
    ...
    try:
        ...
        while readSuccessful:
            ...
            key = cv2.waitKey(10)
            if key == 27 or cv2.getWindowProperty(WINDOW_NAME, 0):  # modified: exit on ESC or Close(X) button too (galal1)
                import os
                path = os.path.join(os.getcwd() + '/datas/lastOutput.png')
                cv2.imwrite(path, frame) # save the last-displayed image to file, for our report
                break
...
def mainForTraining():
    ...
    recordedEvents=0
    HT = None
    isMinimizeNecessary = True  # modified: init for can be "minimized" (galal1)
    try:
        while (readSuccessful and recordedEvents < MAX_SAMPLES_TO_RECORD and not crosshair.userWantsToQuit and
               getIsDoTrainingRunning()):   # modified: to can relaunch from my gui.py (galal1)
    ...
    ##                    HT = fitTransformation(np.array(crosshair.result))
                ...
                if HT is not None: # draw predicted eye position
                    if isMinimizeNecessary == True: # modified: to continue to testing on (gui.py) gui screen (galal1)
                        crosshair.minimizeScreen()
                        isMinimizeNecessary = False
                    currentFeatures = getFeatures( np.array( (pupilOffsetXYList[0], pupilOffsetXYList[1]) ))
                    gazeCoords = currentFeatures.dot(HT)    # modified: to set gazeCoords (galal1)
                    setGazeCoords(gazeCoords[0, 0], gazeCoords[0, 1])
